...
class ArmController(object):
    def __init__(self, robot):
        ...
        self.prs_names = ["head_touch_sensor"]
    ...
    def sweep_action(self, distance_to_wall, height=-0.07):
        ...
        while d_y > 0.4:
            ...
            dq = kinematics.all_joints(
                ...
                self.sec_3_length,
                self.head_length,
                round(self.position_sensors[1].getValue(), 2),
            ...
            while self.robot.step(self.time_step) != -1:
                ...
                self.last_4_positions.append(
                    [
                        self.position_sensors[1].getValue(),
                ...
                if not tuck_in:
                    ...
                    if self.pressure_sensors[0].getValue() < 0.2:
                        ...
                        continue
                if (
                    round(self.position_sensors[1].getValue(), 2) == round(dq[0], 2)
                    and round(self.position_sensors[2].getValue(), 1) == round(dq[1], 1)
                    and round(self.position_sensors[3].getValue(), 1) == round(dq[2], 1)
                    and round(self.position_sensors[4].getValue(), 1) == round(dq[3], 1)
                ):
                    if tuck_in:
    ...
    def tuck_in_action(self):
        ...
        while self.robot.step(self.time_step) != 1:
            if last_joint_set:
                if (
                    round(self.position_sensors[1].getValue(), 2) == round(pos1, 2)
                    and round(self.position_sensors[2].getValue(), 2) == round(pos2, 2)
                    and round(self.position_sensors[3].getValue(), 2) == round(pos3, 2)
                ):
    ...
    def is_stationary(self, last_4_joint_positions):
        ...
        if len(temp) == 4:
            ...
            while len(temp) > 0:
                ...
                if (
                    round(last_joints[0], 3) != round(next_joints[0], 3)
                    or round(last_joints[1], 3) != round(next_joints[1], 3)
                    or round(last_joints[2], 3) != round(next_joints[2], 3)
                ):
                    return False
        ...
        while self.pressure_sensors[0].getValue() < 1.0:
            ...
            while self.robot.step(self.time_step) != -1:
                ...
                if self.pressure_sensors[0].getValue() > 1.0:
                    kin = kinematics.kinematics4joint(
                        self.position_sensors[1].getValue(),
                        self.position_sensors[2].getValue(),
                        self.position_sensors[3].getValue(),
                        self.position_sensors[4].getValue(),
                ...
                if (
                    ...
                    and round(self.position_sensors[2].getValue(), 2) == round(pos2, 2)
                    and round(self.position_sensors[3].getValue(), 2) == round(pos3, 2)
                    and round(self.position_sensors[4].getValue(), 2) == round(pos4, 2)
                ):
    ...
    def set_sweeping_action(self, distance_to_wall, height=-0.20):
        ...
        while self.robot.step(self.time_step) != -1:
            ...
            if (
                ...
                and round(self.position_sensors[3].getValue(), 2) == round(pos3, 2)
                and round(self.position_sensors[4].getValue(), 2) == round(pos4, 2)
            ) or self.is_stationary(self.last_4_positions):
    ...
    def sweep(self, distance_to_wall, table_height=-0.20):
        ...
        distance_to_wall, height = self.set_sweeping_action_sensor(
            distance_to_wall, table_height
        )

...
# can import these as json?
    ...
    def unoccupied_table(self,distance_sensor, distances):
        ...
        if distance_sensor.getValue() < (self.params["DISTANCE_TO_WALL"] - 0.2):
            ...
            self.move_distance((distances[0] / 2) + sum(distances[1:4]) + (distances[4] / 2), 1)
    ...
    def side_check(self,robot, ds):
        ...
        if abs(self._current_side_distance - self._previous_side_distance) > 0.3 and self._enable_updates:
            ...
            if self._current_side_distance > self._previous_side_distance:
                ...
                self._occupied_space = (
                    self.get_distance_since(
                        self._occupied_start, self.params["WHEEL_RADIUS"], self.params["SPEED"], robot
                    )
                ...
                if self.that_a_table(self._distance_morse):  # if a table is found:
                    ...
                    self.move_distance(sum(self._distance_morse[3:]) + (self._distance_morse[2] / 2), -1)
            ...
            # falling edge, distance shrinks, start of chair/pole
                ...
                self._empty_space = (
                    self.get_distance_since(
                        self._empty_start, self.params["WHEEL_RADIUS"], self.params["SPEED"], robot
                    )
                    if (self._occupied_start is not None)
                    else 0
                )

...
MOVE_MULT = (
    0.45  ### constant to slow down move speed ( too high and moving will be inaccurate)
)
TIME_STEP = (
    0  ### constant to control time step ( set to robots basic time step in setup() )
)
NORM_SPEED = 14.8 * MOVE_MULT
WHEEL_RADIUS = 0.05

motor_names = [
    "wheel_1_motor",  # top left
...
def setup(robot):
    ...
    MAX_SPEED = (
        gtop_left_motor.getMaxVelocity()
    )  ### arbitary motor selection for max speed
...
eps = 0.01  ### how close motors position needs to be to target to stop ( necessary so check_move knows when to return )
...
###
def check_move_side_check(
    robot, left_start, right_start, pos, table_check, dist_sensor
):
    left_pos = 0
    right_pos = 0
    ...
    while True:
        ...
        close_to_pos = (
            abs(abs(left_pos - left_start) + abs(right_pos - right_start) - pos * 2)
            < eps
        )
        slowing_down = (
            abs(abs(prev_left_pos - left_pos) + abs(prev_right_pos - right_pos))
            < eps / 2
        )
        if close_to_pos and slowing_down:
            stop(robot)
...
def move_distance_check_sides(robot, dist, table_check, dist_sensor):
    ...
    return check_move_side_check(
        robot, left_offset, right_offset, pos, table_check, dist_sensor
    )
...
def get_offsets():
    return (
        gtop_left_motor.getPositionSensor().getValue(),

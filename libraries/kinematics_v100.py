...
def kinematics4joint(
    theta_1, theta_2, theta_3, theta_4, length1, length2, length3, length4
):
    d_1 = 0
    d_2 = 0
    d_3 = 0
...
def kinematics_button(
    theta_1, theta_2, theta_3, theta_4, length1, length2, length3, length4, length5
):
    d_1 = 0
    d_2 = 0
    d_3 = 0
...
def jacobian(current_theta_1, current_theta_2, l1, l2):
    ...
    j = Matrix(
        [
            [diff(k[3], sym_theta_1), diff(k[3], sym_theta_2)],
            [diff(k[7], sym_theta_1), diff(k[7], sym_theta_2)],
        ]
    )
...
def jacobian3(current_theta_1, current_theta_2, current_theta_3, l1, l2, l3):
    ...
    j = Matrix(
        [
            [diff(k[3], sym_theta_1), diff(k[3], sym_theta_2), diff(k[3], sym_theta_3)],
            [diff(k[7], sym_theta_1), diff(k[7], sym_theta_2), diff(k[7], sym_theta_3)],
        ]
    )
    j = j.subs(sym_theta_1, current_theta_1)
    j = j.subs(sym_theta_2, current_theta_2)
...
def jacobian_just_head(
    current_theta_1, current_theta_2, current_theta_3, current_theta_4, l1, l2, l3, l4
):
    sym_theta_1 = sympy.Symbol("theta_1")
    sym_theta_2 = sympy.Symbol("theta_2")
    sym_theta_3 = sympy.Symbol("theta_3")
    ...
    k = kinematics4joint(
        sym_theta_1, sym_theta_2, sym_theta_3, sym_theta_4, l1, l2, l3, l4
    )
    j = Matrix(
        [
            [
                diff(k[3], sym_theta_1),
                diff(k[3], sym_theta_2),
                diff(k[3], sym_theta_3),
                diff(k[3], sym_theta_4),
            ],
            [
                diff(k[7], sym_theta_1),
                diff(k[7], sym_theta_2),
                diff(k[7], sym_theta_3),
                diff(k[7], sym_theta_4),
            ],
        ]
    )
    j = j.subs(sym_theta_1, current_theta_1)
...
def jacobian_button(
    ...
    sym_theta_4 = sympy.Symbol("theta_4")
    k = kinematics_button(
        sym_theta_1, sym_theta_2, sym_theta_3, sym_theta_4, l1, l2, l3, l4, l5
    )
    j = Matrix(
        [
            [
            ...
            [
                diff(k[11], sym_theta_1),
                diff(k[11], sym_theta_2),
                diff(k[11], sym_theta_3),
                diff(k[11], sym_theta_4),
            ],
        ]
    )
    j = j.subs(sym_theta_1, current_theta_1)
...
def desired_joint_angles(theta1, theta2, l1, l2, theta1_d, theta2_d):
    ...
    dq_d = np.dot(
        J_inv, (np.dot(K_d, error_d.transpose()) + np.dot(K_p, error.transpose()))
...
def big_boi(
    d_x, d_y, len1, len2, len3, len4, theta1=0.0, theta2=0.0, theta3=0.0, theta4=0.0
):
    ...
    while (
        actual_x < desired_x_position - threshold
        or actual_x > desired_x_position + threshold
    ) or (
        actual_y < desired_y_position - threshold
        or actual_y > desired_y_position + threshold
    ):
        angles = desired_joint_angles4(
            theta_1,
            theta_2,
            theta_3,
            theta_4,
...
def big_boi_button(
    ...
    while (
        ...
        or (
            actual_z < desired_z_position - threshold
            or actual_z > desired_z_position + threshold
        )
    ):
        angles = desired_joint_angles_button(
            theta_1,
            theta_2,
            theta_3,
            theta_4,
        ...
        k = kinematics_button(
            angles[0], angles[1], angles[2], angles[3], l1, l2, l3, l4, l5
        )
        actual_x = k[3]
        actual_y = k[7]
        actual_z = k[11]
...
def big_boi_head(
    ...
    while (
        ...
        angles = desired_joint_angles_just_head(
            theta_1,
            theta_2,
            theta_3,
            theta_4,
...
def all_joints(
    ...
    first_3_angles = big_boi_3(
        d_x + len4, d_y, len1, len2, len3, theta1, theta2, theta3
    )
    kine = kinematics3joint(
        first_3_angles[0], first_3_angles[1], first_3_angles[2], len1, len2, len3
    )
    angles = big_boi_head(
        kine[3] - len4,
        kine[7],
...
def joints_button(
    ...
    return big_boi_button(
        d_x, d_y, d_z, len1, len2, len3, len4, len5, theta1, theta2, theta3, theta4
    )
...
def brute_force3(
    d_x, d_y, l1, l2, l3, curr_theta_1=None, curr_theta_2=None, curr_theta_3=None
):
    ...
    for theta3 in np.arange(start_theta_3, end_theta_3, incrementer):
        ...
        actual_y = k[7]
        if (
            actual_x > desired_x_position
            and actual_x < desired_x_position + threshold_x
        ):
            if (
                actual_y > desired_y_position - threshold_y
                and actual_y < desired_y_position + threshold_y
            ):
...
def brute_force(d_x, d_y, l1, l2, curr_theta_1=None, curr_theta_2=None):
    ...
    for theta1 in np.arange(start_theta_1, end_theta_1, incrementer):
        for theta2 in np.arange(start_theta_2, end_theta_2, incrementer):
            ...
            actual_y = k[7]
            if (
                actual_x > desired_x_position - threshold_x
                and actual_x < desired_x_position + threshold_x
            ):
                if (
                    actual_y > desired_y_position - threshold_y
...
def desired_joint_angles_button(
    theta1, theta2, theta3, theta4, l1, l2, l3, l4, l5, x_d, y_d, z_d
):
...
def desired_joint_angles_just_head(
    theta1, theta2, theta3, theta4, l1, l2, l3, l4, x_d, y_d
):
    global error, previous_time
    ...
    for theta1 in np.arange(start_theta_1, end_theta_1, incrementer):
        for theta2 in np.arange(start_theta_2, end_theta_2, incrementer):
            for theta3 in np.arange(start_theta_3, end_theta_3, incrementer):
                ...
                actual_y = k[7]
                if (
                    actual_x > desired_x_position_3
                    and actual_x < desired_x_position_3 + threshold_x_3
                ):
                    if (
                        actual_y > desired_y_position - threshold_y_3
                        and actual_y < desired_y_position + threshold_y_3
                    ):
                        for theta4 in np.arange(
                            start_theta_4, end_theta_4, incrementer
                        ):
                            k = kinematics4joint(
                                theta1, theta2, theta3, theta4, l1, l2, l3, l4
                            )
                            actual_x = k[3]
                            actual_y = k[7]
                            if (
                                actual_x > desired_x_position_head
                                and actual_x
                                < desired_x_position_head + threshold_x_head
                            ):
                                if (
                                    actual_y > desired_y_position - threshold_y_head
                                    and actual_y < desired_y_position + threshold_y_head
                                ):
